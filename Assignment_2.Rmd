...
```{r}
#running pearsons correlation test

cor.test(x = divorce_margarine$divorce_rate_maine, y = divorce_margarine$margarine_consumption_per_capita, method = "pearson")
```
the correlation between divorce rates and margarine consumption is very high at 0.99. Below a linear model will also check the correlation to cross referenece the results. 
...
summary(m1)
```
the results of this linear model along with the correlation test results show that margarine consumption and divorce reate in maine is stringly correlated. The linear model suggets that the baseline for divorce rate 
